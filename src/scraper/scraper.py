...
class UMBScraper:
    def __init__(self):
        ...
        self.config = config_manager.load_bot_config()
    ...
    def get_tournament_details(self, tournament_id: int) -> dict:
        ...
        try:
            response = self.session.get(url, timeout=self.scraper_config['timeout'])
    ...
    def scrape_tournaments(self, start_id: int, end_id: int) -> list:
        ...
        print(f"⏱️  Délai entre requêtes: {self.scraper_config['delay_between_requests']}s")
        print(f"📁 Fichiers de sortie: {self.scraper_config['output_prefix']}.json et {self.scraper_config['output_prefix']}.csv")
        print("-" * 60)
        ...
        for tournament_id in range(start_id, end_id + 1):
            ...
            if tournament_info:
                ...
                try:
                    response = self.session.get(f"{self.base_url}{tournament_id}", timeout=self.scraper_config['timeout'])
    ...
    def save_to_json(self, tournaments: list, filename: str):
        ...
        try:
            # Utiliser le chemin des données
            filepath = config_manager.get_data_path(filename)
            ...
            print(f"✅ Données JSON sauvegardées dans {filepath}")
    ...
    def save_to_csv(self, tournaments: list, filename: str):
        ...
        try:
            ...
            print(f"✅ Données CSV sauvegardées dans {filepath}")
...
def main():
    ...
    json_file = f"{config['output_prefix']}.json"
    json_path = config_manager.get_data_path(json_file)
    
    if json_path.exists():
        print("📁 Fichier JSON existant détecté")
        print("🔄 Mode: SCRAPING SÉLECTIF (optimisé)")
        print(f"  - Fichier de base: {json_file}")
        print(f"  - Plage d'IDs: 1 à {config['end_id']}")
        print(f"  - Délai: {config['delay_between_requests']}s")
        print(f"  - Timeout: {config['timeout']}s")
        print("=" * 60)
        
        # Lancer le scraping sélectif
        tournaments = scraper.selective_scrape(json_file, config['end_id'])
        scraper.print_stats()
        
    else:
        print("📝 Aucun fichier JSON existant")
        print("🔄 Mode: SCRAPING COMPLET")
        print(f"  - Plage d'IDs: {config['start_id']} à {config['end_id']}")
        print(f"  - Délai: {config['delay_between_requests']}s")
        print(f"  - Timeout: {config['timeout']}s")
        print(f"  - Fichiers de sortie: {config['output_prefix']}.json/csv")
        print("=" * 60)
        
        # Lancer le scraping complet
        tournaments = scraper.scrape_tournaments(config['start_id'], config['end_id'])

    # Sauvegarder les résultats
    if tournaments:
        # En mode sélectif, le JSON est déjà sauvegardé dans selective_scrape()
        if not json_path.exists() or not hasattr(scraper, '_selective_mode_used'):
            scraper.save_to_json(tournaments, f"{config['output_prefix']}.json")
        scraper.save_to_csv(tournaments, f"{config['output_prefix']}.csv")

        # Sauvegarder séparément les tournois futurs
        future_tournaments = scraper.filter_future_tournaments(tournaments)
        if future_tournaments:
            scraper.save_to_json(future_tournaments, f"{config['output_prefix']}_future.json")
            scraper.save_to_csv(future_tournaments, f"{config['output_prefix']}_future.csv")
